...
public class EmeraldWallTorch extends WallTorchBlock {
    public EmeraldWallTorch() {
        super(AbstractBlock.Properties.create(Material.MISCELLANEOUS).doesNotBlockMovement().zeroHardnessAndResistance().setLightLevel((state) -> {
            return 15;
        }).sound(SoundType.WOOD).lootFrom(ExtraCoals.EMERALD_TORCH.get()), ParticleTypes.FLAME);
    }
    @Override
    @OnlyIn(Dist.CLIENT)
